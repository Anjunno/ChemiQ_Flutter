...
import '../models/member_signup_request.dart';
import '../models/logout_request.dart';
import 'package:chemiq/core//utils/logger.dart';
...
class AuthRepository {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (accessToken != null && refreshToken.isNotEmpty) {
          await _dioClient.storage.write(key: 'accessToken', value: accessToken);
          await _dioClient.storage.write(key: 'refreshToken', value: refreshToken);
          logInfo('로그인 성공: accessToken과 refreshToken 저장 완료');
      ...
      logError('로그인 실패: ${e.response?.data}');
      rethrow;
    } catch (e) {
      logError('알 수 없는 에러 발생: $e');
      rethrow;
    }
  }
  ...
  /// 로그아웃 API 호출 메서드
  Future<void> logout() async {
    logDebug("[AuthRepository] 로그아웃 메서드 시작.");
    final refreshToken = await _dioClient.storage.read(key: 'refreshToken');
    logDebug("[AuthRepository] 리프레시 토큰 읽기 완료");

    if (refreshToken != null) {
      try {
        final requestDto = LogoutRequest(refreshToken: refreshToken);
        logDebug("[AuthRepository] LogoutRequest DTO 생성 완료");
        await _dioClient.dio.post(
          '/logout',
          data: requestDto.toJson(),
          options: Options(
            extra: {'isLogoutRequest': true}, // 401 처리 회피
          ),
        );
        logInfo('서버 DB의 Refresh Token 무효화 성공');
      } on DioException catch (e) {
        logWarning('서버 로그아웃 요청 실패 (무시함): $e');
      }
    } else {
      logDebug("[AuthRepository] 리프레시 토큰 없음");
    }
  }
